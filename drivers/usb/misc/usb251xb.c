...
static void usb251xb_reset(struct usb251xb *hub, int state)
{
	...
	if (state)
		usleep_range(1000, 1250);       /* >=1000us at power on */
	else
...
static int usb251xb_connect(struct usb251xb *hub)
{
	...
	if (!hub->i2c_enabled) {
		dev_info(dev, "Auto hub configured.\n");
		return 0;
	}
	...
	for (i = 0; i < (USB251XB_I2C_REG_SZ / USB251XB_I2C_WRITE_SZ); i++) {
		...
		char rbuf[USB251XB_I2C_WRITE_SZ + 1];
		...
		err = i2c_smbus_read_i2c_block_data(hub->i2c, offset,
							USB251XB_I2C_WRITE_SZ + 1,
							rbuf);

		for (j = 0; j < USB251XB_I2C_WRITE_SZ; j++) {
			read_offset = offset + j;
			dev_dbg(dev, "Address : 0x%02X, value : 0x%02X\n",
					read_offset, rbuf[j]);
		}
	}
			       ...
			       struct usb251xb_data *data)
{
	...
	if (of_get_property(np, "i2c-config", NULL))
		hub->i2c_enabled = 1;
	else
		hub->i2c_enabled = 0;
